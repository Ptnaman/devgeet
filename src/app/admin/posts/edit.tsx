...
export default function AdminPostEditScreen() {
  ...
  const hasResetInitialContentSelectionRef = useRef(false);
  ...
  }, [canModeratePosts, isEditing, postId, router, user?.email, user?.uid]);
  ...
  useEffect(() => {
    if (!isEditing || isLoadingPost) {
      hasResetInitialContentSelectionRef.current = false;
      return;
    }

    if (!contentPlainText || hasResetInitialContentSelectionRef.current) {
      return;
    }
    ...
    const frame = requestAnimationFrame(() => {
      ...
      hasResetInitialContentSelectionRef.current = true;
    });

    return () => {
      cancelAnimationFrame(frame);
    };
  }, [contentPlainText, isEditing, isLoadingPost]);
